...
function readableError(error) {
  const messages = {
    ...
    'auth/configuration-not-found': 'Firebase Email/Password sign-in has not been enabled yet.',
    ...
    'auth/api-key-not-valid': 'The Firebase web API key is invalid. Check the VITE_FIREBASE_API_KEY value in .env.local.',
  ...
  return messages[error.code] || `Firebase could not complete that request${error.code ? ` (${error.code})` : ''}.`
}
...
export function AuthForm({ onClose, required = false }) {
  ...
  return <div className={required ? 'auth-backdrop auth-required' : 'auth-backdrop'} role="presentation" onMouseDown={required ? undefined : onClose}>
    <section className="auth-dialog" role="dialog" aria-modal="true" aria-labelledby="auth-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <p className="auth-intro">{mode === 'signin' ? 'Sign in to access your saved opportunities.' : 'Register to save donor opportunities and keep your workspace in sync.'}</p>
      <form className="auth-form" onSubmit={submit}>
        ...
        <button className="primary-button auth-submit" disabled={submitting}>{submitting ? 'Please wait…' : mode === 'signin' ? 'Sign in' : 'Create account'}</button>
      </form>
      <p className="auth-switch">{mode === 'signin' ? 'New to mHub?' : 'Already have an account?'} <button onClick={changeMode}>{mode === 'signin' ? 'Create an account' : 'Sign in'}</button></p>
    </section>
  </div>
}
