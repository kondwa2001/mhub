...
export function CollaboratorDirectory({ collaborators, isSampleData, onAdded }) {
  ...
  const submit = async (event) => {
    ...
    if (!form.name.trim()) return setError('Give the collaborator a name.')
    ...
    try {
      ...
      setError(saveError?.message || 'Could not save this collaborator. Please try again.')
    } finally {
      setSaving(false)
    }
  }
      ...
      {isSampleData && (
        <div className="collab-notice">
          ...
          <p>
            ...
            Nothing has been saved to the database yet — add a real collaborator below and the
      ...
      {open && (
        <form className="collab-form" onSubmit={submit}>
          ...
          <button className="collab-submit" disabled={saving}>
            <Icon name="save" />
            {saving ? 'Saving…' : 'Save collaborator'}
          </button>
        </form>
      )}
