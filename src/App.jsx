...
function App() {
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      <nav aria-label="Main navigation">{['Overview', 'Donor finder', 'Activities', 'Saved'].map((item) => <button key={item} className={tab === item ? 'nav-item active' : 'nav-item'} onClick={() => selectNavigation(item)}><Icon name={item === 'Overview' ? 'grid' : item === 'Donor finder' ? 'search' : item === 'Activities' ? 'calendar' : 'bookmark'} />{item}{item === 'Saved' && saved.size > 0 && <b>{saved.size}</b>}</button>)}</nav>
      <div className="sidebar-bottom"><button className="nav-item"><Icon name="settings" />Settings</button><button className="nav-item logout-button" onClick={signOut}><Icon name="logout" />Log out</button><div className="profile"><div className="avatar">{user.name?.[0] || 'M'}</div><div><strong>{user.name || 'MHub team'}</strong><small>{user.email}</small></div></div></div>
    </aside>
